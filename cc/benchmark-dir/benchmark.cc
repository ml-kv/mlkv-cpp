...
class AtomicGenLock {
 public:
  ...
  inline bool try_lock(int32_t staleness_incr, int32_t staleness_bound) {
  ...
  inline void unlock() {
    ...
    uint64_t sub_delta = ((uint64_t)1 << 63) - ((uint64_t)1 << 32);
...
class UpsertContext : public IAsyncContext {
 public:
  ...
  inline bool PutAtomic(value_t& value) {
    while(!value.gen_lock_.try_lock(/*staleness_incr*/ 0, /*staleness_bound*/ INT32_MAX)) {
      std::this_thread::yield();
    }
    value.atomic_value_.store(input_);
    value.gen_lock_.unlock();
    return true;
  }
...
class MLKVReadContext : public IAsyncContext {
 public:
  ...
  MLKVReadContext(uint64_t key, int32_t staleness_incr, int32_t staleness_bound)
    : key_{ key }
  ...
  inline void RmwCopy(const value_t& old_value, value_t& value) {
    GenLock old_gen_lock_ = old_value.gen_lock_.load();
    GenLock new_gen_lock_;
    new_gen_lock_.staleness = old_gen_lock_.staleness + staleness_incr_;
    value.gen_lock_.store(new_gen_lock_);
    value.value_ = old_value.value_;
  }
  inline bool RmwAtomic(value_t& value) {
    while(!value.gen_lock_.try_lock(staleness_incr_, staleness_bound_)) {
      std::this_thread::yield();
    }
    output_ = value.atomic_value_.load();
    value.gen_lock_.unlock();
    return true;
  }
...
class MLKVUpsertContext : public IAsyncContext {
 public:
  ...
  MLKVUpsertContext(uint64_t key, uint64_t input,
...
class MLKVLookaheadContext : public IAsyncContext {
 public:
  ...
  MLKVLookaheadContext(uint64_t key)
    : key_{ key } {
  }
    ...
    for(uint64_t idx = chunk_idx; idx < chunk_idx + kChunkSize; ++idx) {
      ...
      case Op::Upsert: {
        ...
        */
        ...
        MLKVUpsertContext context{ txn_keys_.get()[idx], upsert_value, -1, INT32_MAX };
	...
	++writes_done;
      ...
      case Op::Read: {
        ...
        */
        ...
        MLKVReadContext context{ txn_keys_.get()[idx], 1, INT32_MAX };
        Status result = store->Rmw(context, callback, 1);
        ++reads_done;
        break;
      }
      case Op::ReadModifyWrite:
        ...
        RmwContext context = RmwContext{ txn_keys_.get()[idx], 5 };
        Status result = store->Rmw(context, callback, 1);
        if(result == Status::Ok) {
          ++writes_done;
        }
