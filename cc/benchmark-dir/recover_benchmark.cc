...
void run(Workload workload, size_t num_threads) {
  ...
  store_t new_store{ init_size, 256 * 1024 * 1024, "storage", 0.5 };
  ...
  for(const auto& recovered_session_id : recovered_session_ids) {
    serial_nums.push_back(new_store.ContinueSession(recovered_session_id));
  ...
  case Workload::A_50_50:
    run_benchmark<ycsb_a_50_50>(&new_store, num_threads);
    break;
  case Workload::RMW_100:
    run_benchmark<ycsb_rmw_100>(&new_store, num_threads);
    break;
  default:
